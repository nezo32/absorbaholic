...
3. `github-release`: the release "Absorbaholic X.Y.Z", with generated notes and two assets:
   `absorbaholic-X.Y.Z.jar` and `absorbaholic-X.Y.Z-sources.jar`.
4. `curseforge-mod`: uploads the jar as "Absorbaholic X.Y.Z (Fabric)" with the sources jar as a child file and
   Fabric API as a required dependency, using the release notes as the changelog.
...
| `actionlint` | `ci.yml` |
| `scripts` | `ci.yml` (shellcheck, upload-script tests, inlined-script check, reusable-workflow mirror check) |
...
```bash
gh api -X PUT repos/nezo32/absorbaholic/branches/main/protection --input - <<'JSON'
{
...
Settings → Secrets and variables → Actions. The CurseForge token and project id may be repository-level **or** live in the GitHub Environment `Absorbaholic` (the upload job runs in it; override the name with the repository variable `CURSEFORGE_ENVIRONMENT`). A missing project id shows up as a warning in the run, not as a silently skipped job.

| Kind | Name | Value |
|---|---|---|
| secret | `CURSEFORGE_TOKEN` | CurseForge Authors portal → API tokens |
| variable | `CURSEFORGE_PROJECT_ID` | numeric id of the Absorbaholic CurseForge project (sidebar of the project page). Empty = CurseForge skipped |
| variable (optional) | `CURSEFORGE_GAME_VERSIONS` | default `26.2,26.3,Fabric,Java 25,Client,Server` |
| variable (optional) | `CURSEFORGE_ENVIRONMENT` | repository-level only; name of the GitHub Environment the upload job runs in. Default `Absorbaholic` |

These are all the names the workflows read. The Fabric API relation (`fabric-api:requiredDependency`) and the display
name are set in `release.yml` itself.
...
The defaults were verified against the live API with a real token (September 2026, for Enchantaholic, on the same
host and versions):
...
Push `v0.0.1-alpha.1` on `main` while `CURSEFORGE_PROJECT_ID` is still unset: you get a GitHub pre-release with the
jars, and the CurseForge job is skipped. Then set the variable and run the workflow with `curseforge-dry-run: true`
for that tag before the first real release.
