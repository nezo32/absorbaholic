...
The pipeline is five reusable workflows (`on: workflow_call`) hosted in
[nezo32/enchantaholic](https://github.com/nezo32/enchantaholic). They contain nothing Enchantaholic-specific: every
project detail comes from the calling workflow's inputs and from repository variables.

> **Absorbaholic: mirrored pipeline.** This repository keeps **byte-identical mirrors** of the five
> `reusable-*.yml` files in `.github/workflows/`, taken from `nezo32/enchantaholic` at revision
> [`a3e3f7d4160e692a64845c196922c780d1608f9f`](https://github.com/nezo32/enchantaholic/tree/a3e3f7d4160e692a64845c196922c780d1608f9f).
> Absorbaholic's `ci.yml` and `release.yml` call the mirrors **by local path**
> (`uses: ./.github/workflows/reusable-version.yml`), exactly as Enchantaholic calls its own copies, so CI and releases
> never depend on an upstream change that has not been pushed yet. Everything below describes the shared pipeline and
> applies unchanged; only the `uses:` prefix differs from the [Quick start](#2-quick-start).
>
> - **Do not edit the mirrors here.** Change the workflow in `nezo32/enchantaholic` first, then copy the new files
>   over and set `UPSTREAM_SHA` in [`scripts/check-reusable-mirror.sh`](../../scripts/check-reusable-mirror.sh) (and
>   the revision above, and in the header of `release.yml`) to the new upstream commit.
> - **CI guard.** The `scripts` job runs `scripts/check-reusable-mirror.sh`, which downloads each
>   `reusable-*.yml` from `raw.githubusercontent.com` at `UPSTREAM_SHA` and fails on any difference.
>   `scripts/curseforge-upload.sh` stays byte-identical as well; `check-inlined-script.sh` compares it with the heredoc.
> - **Switching to the remote call.** Replace each local reference with the pinned upstream one, for example
>   ```yaml
>   # before
>   uses: ./.github/workflows/reusable-version.yml
>   # after
>   uses: nezo32/enchantaholic/.github/workflows/reusable-version.yml@a3e3f7d4160e692a64845c196922c780d1608f9f
>   ```
>   (the same for `reusable-build-gradle.yml`, `reusable-github-release.yml` and `reusable-publish-curseforge.yml`),
>   then delete the local `reusable-*.yml` and the `check-reusable-mirror.sh` step. Bump the SHA deliberately when you
...
CurseForge project. Enchantaholic's own
[`release.yml`](https://github.com/nezo32/enchantaholic/blob/a3e3f7d4160e692a64845c196922c780d1608f9f/.github/workflows/release.yml) is a working example
...
```
See Enchantaholic's [`ci.yml`](https://github.com/nezo32/enchantaholic/blob/a3e3f7d4160e692a64845c196922c780d1608f9f/.github/workflows/ci.yml), or
Absorbaholic's [`ci.yml`](../../.github/workflows/ci.yml) (Fabric only).
