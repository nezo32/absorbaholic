 ...
 */
public record SourceSummary(Identifier id, SourceTargets targets, Optional<Identifier> icon, int color, Tier tier, int maxLevel,
		String traitKey, String weaknessKey) implements SourceLike {
	...
	public static SourceSummary of(SourceDefinition d) {
		return new SourceSummary(d.id(), d.targets(), d.icon(), d.color(), d.tier(), d.maxLevel(), d.trait().key(), d.weakness().key());
	}
