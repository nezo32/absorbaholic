# Contributing to Absorbaholic

Absorbaholic is a Java-edition Fabric mod (Minecraft 26.2–26.3, Java 25) in `fabric/`. It is built, tested and
released by GitHub Actions from this repository.
...
main ──●──────────●────────────●──── tag v1.3.0 → release
        \        / squash     /
         feature/obsidian-trait  fix/absorb-cooldown
```
...
Name your branch `<type>/<kebab-name>`, for example `feature/obsidian-trait` or `fix/absorb-cooldown`. The
...
sentence that makes sense to players: "Add Obsidian trait", not "obsidian wip".

- Add the `breaking` label to a PR that breaks worlds, configs, datapacks or compatibility. It gets its own section at
  the top.
...
```bash
(cd fabric && ./gradlew build)             # Java 25; compiles, runs JUnit + server GameTests, builds the jar (26.3)
...
bash scripts/check-reusable-mirror.sh                                     # needs network access
actionlint                                                                # https://github.com/rhysd/actionlint
actionlint .github/templates/release-caller.yml
```
...
- The `reusable-*.yml` workflows are byte-identical mirrors of the shared pipeline in
  [nezo32/enchantaholic](https://github.com/nezo32/enchantaholic); other projects use them too. Do not edit them here:
  change them upstream first, then re-mirror them (`check-reusable-mirror.sh` fails while they differ from the pinned
  upstream revision). See [docs/ci/REUSABLE_RELEASE_PIPELINE.md](docs/ci/REUSABLE_RELEASE_PIPELINE.md).
